...
#ifndef STDP_DOPAMINE_SYNAPSE_EXP_H
#define STDP_DOPAMINE_SYNAPSE_EXP_H
 ...
 * connection.
 */
class STDPDopaCommonPropertiesExp : public CommonSynapseProperties
{
...
inline long
STDPDopaCommonPropertiesExp::get_vt_node_id() const
{
...
class stdp_dopamine_synapse_exp : public Connection< targetidentifierT >
{
...
public:
  ...
  check_connection( Node& s, Node& t, size_t receptor_type, const CommonPropertiesType& cp )
  {
    ...
    t.register_stdp_connection( t_lastspike_ - get_delay_steps(), get_delay_steps() );
  }
...
private:
  ...
  std::vector< double > exp_tau_n_;
  std::vector< double > exp_tau_c_;
  std::vector< double > exp_tau_plus_;
  std::vector< double > exp_taus_;
...
stdp_dopamine_synapse< targetidentifierT >::send( Event& e, size_t t, const STDPDopaCommonProperties& cp )
{
  ...
  long dendritic_delay = get_delay_long();
  ...
  const STDPDopaCommonProperties& cp )
{
  ...
  // purely dendritic delay
  long dendritic_delay = get_delay_steps();
...
#endif // of #ifndef STDP_DOPAMINE_SYNAPSE_EXP_H
